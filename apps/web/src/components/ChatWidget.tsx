    ...
    <div className="mb-5">
      <div
        className={cn(
          ...
          focused
            ? 'border border-black/30 shadow-[var(--shadow-card)]'
      ...
      <div className="flex gap-1.5 mt-2 flex-wrap">
        {['What deals need follow-up?', 'Summarize Mlhuillier pipeline', 'Who has the highest win rate?'].map((prompt) => (
          <button
            ...
            className="px-[10px] py-1 rounded-full border border-black/[.06] dark:border-white/[.08] bg-card text-xxs font-medium text-slate-600 dark:text-slate-400 hover:border-slate-300 hover:text-slate-900 dark:text-white transition-colors duration-150"
          >
            {prompt}
          </button>
        ))}
