...
import { formatBrandName, getBrandColor, getInitials } from '@/lib/utils'
...
function WikiEmpty({ companies, deals }: { companies: ApiCompanyDetail[]; deals: ApiDeal[] }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-6 text-center">
      <div className="w-12 h-12 rounded-md bg-primary/[.08] dark:bg-primary/[.12] flex items-center justify-center mb-4">
      ...
      <h2 className="text-sm font-semibold text-foreground mb-1">Wiki</h2>
      <p className="text-xs text-muted-foreground max-w-[220px] leading-relaxed mb-6">
    ...
    <div className="flex flex-col h-full">
      {onBack && (
        <div className="px-4 pt-3 pb-0 shrink-0 md:hidden">
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-700 hover:text-foreground transition-colors"
          >
      ...
      <div className="px-5 pt-5 pb-4 border-b border-border shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-sbase font-semibold text-foreground truncate">{formatBrandName(company.name)}</h2>
            {company.industry && (
            ...
            {deals.map(deal => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors hover:bg-surface-hover"
                >
                  <div className="w-2 h-2 rounded-full shrink-0" style={{ background: stageColor }} />
                  <span className="flex-1 text-xs font-medium text-foreground truncate">{deal.title}</span>
                </button>
              )
            })}
          </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-3.5 border-b border-border shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-ssm font-semibold text-foreground truncate leading-snug">
              {deal.title}
            </h2>
            {company && (
              <span className="text-xxs text-muted-foreground">{formatBrandName(company.name)}</span>
            )}
          </div>
  ...
  if (isLoading) {
    return (
      <div className="px-6 pt-6 space-y-2">
        {[1, 2, 3].map(i => (
          <div key={i} className="animate-pulse h-4 rounded bg-secondary" />
        ))}
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="px-6 py-4 border-b border-border shrink-0">
          <h1 className="text-base font-semibold text-foreground leading-snug break-words">
            {filename}
          </h1>
        </div>
        <iframe
...
function MarkdownDocInline({ doc }: { doc: ApiDocument }) {
  ...
  if (isLoading) {
    return <div className="space-y-2">{[1, 2, 3].map(i => <div key={i} className="animate-pulse h-4 rounded bg-secondary" />)}</div>
  }
  ...
  return (
    <div className="rounded-lg border border-border bg-surface-alt p-6">
      ...
      <a
        ...
        download={filename}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-border text-muted-foreground hover:bg-surface-hover transition-colors"
      >
    ...
    <div className="px-8 py-6 max-w-[820px] mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          <span
            className="px-2 py-0.5 rounded eyebrow-label"
...
function LogView({ log }: { log: string | null }) {
  ...
  return (
    <div className="px-6 py-5 max-w-[820px] mx-auto">
      <div className="mb-4">
        <span
          className="inline-block px-2 py-0.5 rounded eyebrow-label"
          style={{ background: 'rgba(100,116,139,0.08)', color: '#64748b' }}
        >
          log
        </span>
      </div>
      <div className="space-y-2 rounded-lg bg-surface-alt px-4 py-3">
        {log.split('\n').filter(Boolean).map((line, i) => (
          <p key={i} className="text-ssm leading-7 text-muted-foreground font-mono">
            {line}
          </p>
        ))}
      </div>
...
function renderSimpleMarkdown(content: string): React.ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    if (line.trimStart().startsWith('```')) {
      ...
      nodes.push(
        <pre key={`code-${i}`} className="my-3 px-4 py-3 rounded-md bg-secondary border-l-2 border-border overflow-x-auto">
          <code className="text-ssm font-mono text-muted-foreground leading-6 whitespace-pre">
            {codeLines.join('\n')}
          </code>
        </pre>,
      )
    ...
    if (line.startsWith('## ')) {
      nodes.push(
        <h2 key={i} className="text-xl font-semibold text-foreground mt-6 mb-2.5">
    ...
    if (line.startsWith('- ') || line.startsWith('* ')) {
      ...
      nodes.push(
        <ul key={`ul-${i}`} className="my-2 space-y-1.5 pl-1">
          {items.map((it, j) => (
            <li key={j} className="flex gap-2 text-base text-muted-foreground leading-7">
              <span className="shrink-0 mt-[10px] w-1.5 h-1.5 rounded-full bg-slate-400" />
    ...
    if (NUMBERED_RE.test(line)) {
      ...
      nodes.push(
        <ol key={`ol-${i}`} start={startNum} className="my-2 space-y-1.5 pl-7 list-decimal marker:text-slate-400 dark:marker:text-slate-500 marker:tabular-nums">
          {items.map((it, j) => (
            <li key={j} className="text-base text-muted-foreground leading-7 pl-1">
              {inlineFormat(it)}
            </li>
          ))}
        </ol>,
    ...
    nodes.push(
      <p key={i} className="text-base text-muted-foreground leading-7">
        {inlineFormat(line)}
      </p>,
    )
    i++
...
function inlineFormat(text: string): React.ReactNode {
  ...
  while ((match = codeRe.exec(text)) !== null) {
    ...
    parts.push(
      <code key={key++} className="px-1.5 py-0.5 rounded text-[0.9em] font-mono bg-secondary text-foreground">
        {match[1]}
      </code>,
    )
...
function boldFormat(text: string, baseKey: number): React.ReactNode {
  ...
  return parts.map((part, i) =>
    i % 2 === 1
      ? <strong key={`${baseKey}-${i}`} className="font-semibold text-foreground">{part}</strong>
