...
export type CreatePartnerDealGroupDto = {
  name: string
  slug?: string
...
export type UpdatePartnerDealGroupDto = {
  name?: string
  slug?: string
  description?: string | null
  isActive?: boolean
...
@Injectable()
export class PartnerDealGroupsService {
  constructor(
  ...
  async findOne(id: string) {
    const rows = await this.db
      ...
      .leftJoin(partnerDealGroupMembers, eq(partnerDealGroupMembers.groupId, partnerDealGroups.id))
      .leftJoin(users, eq(users.id, partnerDealGroupMembers.userId))
      .where(eq(partnerDealGroups.id, id))
  ...
  async create(dto: CreatePartnerDealGroupDto, performedBy?: string) {
    ...
    const [group] = await this.db.insert(partnerDealGroups).values({
    ...
    if (memberUserIds.length > 0) await this.replaceMembers(group.id, memberUserIds, performedBy, group.workspaceId)

    this.auditLogs.log({
      action: 'create',
      auditType: 'partner_deal_group_created',
      entityType: 'partner_deal_group',
      entityId: group.id,
      performedBy,
  ...
  async update(id: string, dto: UpdatePartnerDealGroupDto, performedBy?: string) {
    const existing = await this.findOne(id)
    if (!existing) throw new NotFoundException('Partner deal group not found')
    const memberUserIds = normalizeStringArray(dto.memberUserIds, 'memberUserIds')

    const updates: Partial<typeof partnerDealGroups.$inferInsert> = { updatedAt: new Date() }
    ...
    await this.db.update(partnerDealGroups).set(updates).where(eq(partnerDealGroups.id, id))
    if (memberUserIds) await this.replaceMembers(id, memberUserIds, performedBy, existing.workspaceId)

    this.auditLogs.log({
      action: 'update',
      auditType: 'partner_deal_group_updated',
      entityType: 'partner_deal_group',
      entityId: id,
      performedBy,
  ...
  async addUserToGroups(userId: string, groupIds: string[], performedBy?: string) {
    ...
    const groupRows = await this.db
      ...
      .from(partnerDealGroups)
      .where(and(
        inArray(partnerDealGroups.id, uniqueGroupIds as [string, ...string[]]),
        eq(partnerDealGroups.isActive, true),
      ))
    const validGroupIds = new Set(groupRows.map(group => group.id))
    const invalidIds = uniqueGroupIds.filter(id => !validGroupIds.has(id))
    if (invalidIds.length > 0) throw new BadRequestException('partnerDealGroupIds must contain active partner deal groups only')

    await this.db.insert(partnerDealGroupMembers).values(groupRows.map(group => ({
  ...
  async remove(id: string, performedBy?: string) {
    const existing = await this.findOne(id)
    if (!existing) throw new NotFoundException('Partner deal group not found')
    await this.db.update(partnerDealGroups).set({ isActive: false, updatedAt: new Date() }).where(eq(partnerDealGroups.id, id))
    this.auditLogs.log({
      action: 'delete',
      auditType: 'partner_deal_group_deactivated',
      entityType: 'partner_deal_group',
      entityId: id,
      performedBy,
  ...
  private async replaceMembers(groupId: string, memberUserIds: string[], performedBy?: string, workspaceId?: string | null) {
    const uniqueUserIds = normalizeStringArray(memberUserIds, 'memberUserIds') ?? []
    if (uniqueUserIds.length > 0) {
      const partnerRows = await this.db
        .select({ id: users.id })
        .from(users)
        .where(and(
          inArray(users.id, uniqueUserIds as [string, ...string[]]),
          eq(users.role, CrmUserRole.Partner),
          eq(users.status, CrmUserStatus.Active),
          eq(users.isActive, true),
          isNull(users.deletedAt),
        ))
      const validUserIds = new Set(partnerRows.map(user => user.id))
      const invalidIds = uniqueUserIds.filter(id => !validUserIds.has(id))
      if (invalidIds.length > 0) throw new BadRequestException('memberUserIds must contain active PARTNER users only')
    }

    await this.db.delete(partnerDealGroupMembers).where(eq(partnerDealGroupMembers.groupId, groupId))
    if (uniqueUserIds.length === 0) return

    await this.db.insert(partnerDealGroupMembers).values(uniqueUserIds.map(userId => ({
